...
/** Reuses Create's own Deployer BER arm/hand partial rendering inside Sable. */
@Mixin(targets = "com.simibubi.create.content.kinetics.deployer.DeployerRenderer", remap = false)
public class DeployerRendererMixin {
    private static final Set<String> LOGGED_DEPLOYER_BER = Collections.newSetFromMap(new ConcurrentHashMap<>());
    ...
    private boolean sable$allowDeployerVanillaBer(final LevelAccessor level, final Operation<Boolean> original,
                                                  final DeployerBlockEntity blockEntity, final float partialTick,
        ...
        if (LOGGED_DEPLOYER_BER.add(key)) {
            Sable.LOGGER.info("SABLE_M18_DEPLOYER_RENDER stage=STATIC_BER_VISUAL_DECISION renderer={} blockEntityClass={} blockId={} pos={} sableSubLevel={} originalVisualizationSupported={} returnedVisualizationSupported={} kineticSpeed={} handOffset={} hiddenPlotPoseTranslation=false renderPath=Create_DeployerRenderer_renderComponents",
                    ...
                    blockEntity.getHandOffset(partialTick));
        }
